...
/**
 * Postgres datasource authentication library
 */
...
$sumo_verify_datasource_connection = create_function('$id=false', '
	...
	{		
		...
		if(!$datasource["port"]) $datasource["port"] = 5432;
...
$sumo_verify_datasource_authentication = create_function('$id=FALSE', ' 
	...
	{			
		...
		if($db->IsConnected()) 
		{ 	
			$query = "SELECT ".$datasource["db_field_password"]." FROM public.".$datasource["db_table"]." 
